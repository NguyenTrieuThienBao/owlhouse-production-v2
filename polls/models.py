...
class WebsiteStats(models.Model):
    ...
    def __str__(self):
        return f"Website Stats - {self.timestamp}"
    ...
    def calculate_total_visits():
        ...
        return total_visits
    @staticmethod
    def calculate_total_registered_users():
        total_registered_users = CUser.objects.count()
        logging.debug(f"Total registered users: {total_registered_users}")
        return total_registered_users

def save(self, *args, **kwargs):
    self.total_visits += self.calculate_total_visits()
    self.total_registered_users = WebsiteStats.calculate_total_registered_users()
    self.total_sales += self.calculate_total_sales()
    self.total_sold_products += WebsiteStats.calculate_total_sold_products()
